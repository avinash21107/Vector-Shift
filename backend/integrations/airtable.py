...
async def oauth2callback_airtable(request: Request):
    ...
    # NOTE: code_verifier was saved as a string; do not call .decode()
    # if code_verifier is None this will also raise, which is fine.
        ...
        response, _, _ = await asyncio.gather(
            client.post(
                ...
                data={
                    ...
                    'client_id': CLIENT_ID,
                    'code_verifier': code_verifier,  # <- no .decode()
                },
                headers={
